...
import dev.vality.dudoser.service.PartyManagementService;
import dev.vality.dudoser.service.PaymentPayerService;
...
public class PaymentPayerServiceImpl implements PaymentPayerService {
    ...
    public PaymentPayer convert(Invoice invoiceWrapper, String invoiceId, String paymentId) {
        ...
        if (paymentTool.isSetBankCard()) {
            ...
            cardType = paymentTool.getBankCard().getPaymentSystem().getId();
        }
    ...
    private dev.vality.damsel.domain.InvoicePayment getInvoicePayment(
            Invoice invoice,
            String invoiceId,
            String paymentId
    ) {
        ...
        return invoice.getPayments().stream()
                ...
                .map(
                        invoicePayment -> invoicePayment.getRefunds().stream()
                                .map(dev.vality.damsel.payment_processing.InvoicePaymentRefund::getRefund)
    ...
    private String getShopUrl(dev.vality.damsel.domain.Invoice invoice) {
        String shopUrl;
    ...
    private Content getMetadata(dev.vality.damsel.domain.InvoicePayment invoicePayment) {
    ...
    private Content getInvoiceMetadata(dev.vality.damsel.domain.Invoice invoice) {
